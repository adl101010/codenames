...
A prebuilt image is also published to GHCR on every push to `master` (see `.github/workflows/docker-ghcr.yml`); `docker-compose.yml` pulls `ghcr.io/adl101010/codenames:latest` directly, no local build required. The image builds `linux/amd64` only.
...
- **A live-toggleable timer.** Configured from the in-game settings menu (gear icon) rather than only at game creation, so a timer can be turned on mid-game -- useful if a game starts casually and turns need to start being enforced. Includes a duration and an "enforce timer" option (auto-ends the turn on expiry vs. just visual pressure).
...
- Next Game button always asks for confirmation before resetting the board; that confirmation (and the clue-giving confirmation) is sized for touchscreen use -- both were specifically enlarged for use on a tablet.
- Assorted contrast/legibility fixes: revealed blue tiles lightened slightly so black text stays readable against them, timer and settings ON/OFF text reinforced to actually render bold (the monospace font's "bold" weight was barely distinguishable from regular), turn badge centering, full-screen mode hides the top bar.
